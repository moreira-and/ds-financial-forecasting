...
from src.infrastructure.config.settings import MLFLOW_TRACKING_URI
from src.infrastructure.adapters.predict.py_func_model_builder import PyFuncModelTemplate
...
class LSTMRegressionExperiment(IExperimentTemplate):
    ...
    def run(self):
        ...
        with mlflow.start_run() as run:
            ...
            artifacts = {"model": self.model_path, "preprocessor": self.preprocessor_path, "postprocessor": self.postprocessor_path}
            ...
            mlflow.pyfunc.log_model(
                ...
                signature="infer",
                pip_requirements="infer",
            )

    def _build_pyfunc_model(self):
        return self.py_func(preprocessor=self.preprocessor, model=self.model, postprocessor=self.postprocessor)
