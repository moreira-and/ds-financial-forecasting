...
from src.infrastructure.adapters.features.splitter_strategy import ISplitterStrategy
from src.infrastructure.adapters.features.transform_strategy import ITransformStrategy
from src.infrastructure.adapters.features.generator_strategy import IGeneratorStrategy
from src.infrastructure.adapters.features.preprocessor_strategy import DefaultRnnPreprocessor
...
class DefaultRnnPrepareDataTemplate(IPrepareDataTemplate):
    ...
    def prepare_data(self):
        ...
        if self.targets is None or len(self.targets) == 0:
            ...
            y_features = [(i, feature) for i, feature in enumerate(feature_names) if any(t in feature for t in self.targets)]
    ...
    def get_postprocessor(self):
        ...
        if self.targets is None or len(self.targets) == 0:
            ...
            filtered_columns = [feature for feature in X if any(t in feature for t in self.targets)]
