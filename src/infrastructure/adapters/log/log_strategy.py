...
from src import config as config_pkg  # noqa: F401
from src.infrastructure.config import settings as config
from src.infrastructure.config.settings import logger
from src.infrastructure.adapters.log.python_model_pipeline import PythonModelPipeline
...
class KerasExperimentMlFlowLogger(ILogStrategy):
    def __init__(self, model, history, validation_len, batch_size, elapsed_time=None):
        self.model = model
    ...
    def log_Keras(self, model_name):
        ...
        mlflow.keras.log_model(self.model, artifact_path="artifacts/model", registered_model_name=model_name, signature=signature)
    ...
    def run(self, run_name="training_run", experiment_name="default_experiment", model_name="regression-pipeline", purpose_tag="regression-pipeline"):
